...
class SparseGCNConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.lin = sl.SparseLinear(in_channels, out_channels, sparsity=0.7, bias=False)
...
class SparseCRD(torch.nn.Module):
    def __init__(self, d_in, d_out, p):
        super(SparseCRD, self).__init__()
        self.conv = SparseGCNConv(d_in, d_out, cached=True, bias=False)
        self.p = p
...
class SparseCLS(torch.nn.Module):
    def __init__(self, d_in, d_out):
        super(SparseCLS, self).__init__()
        self.conv = GCNConv(d_in, d_out, cached=True, bias=False)
...
class SparseNet(torch.nn.Module):
    def __init__(self, dataset, hidden, dropout):
        ...
        self.crd = SparseCRD(dataset.num_features, hidden, dropout)
        self.cls = SparseCLS(hidden, num_classes)
